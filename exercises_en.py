...
def get_streams_chart(artist, title, rank_1_only):
    ...
    # consider all songs that were at least once #1
...
app.layout = dmc.MantineProvider([
    ...
    dmc.Group([
        ...
        # Switch for the option to display only songs that were at least once #1
...
def update_streams_per_month(selected_artist, selected_title, rank_1_only):
    # only consider songs that were at least once #1
    tmp_df = df.filter(pl.col("rank").eq(1)) if rank_1_only else df
    # possible titles for the current filter
    ...
    # possible artists for the current filter
